          ...
          {PLATFORMS.map(p => (
            <button
              ...
              className={`h-9 px-4 rounded-full text-[14px] font-medium transition-all active:opacity-70
                ${platform === p ? 'bg-[#6366F1] text-white' : 'border border-stone-200 text-stone-600 bg-white'}`}
            >
              {p}
            </button>
          ))}
          ...
          {profiles.map(inf => {
            ...
            return (
              ...
              >
                ...
                {/* Radio button */}
                <div className={`w-[22px] h-[22px] rounded-full border-2 flex items-center justify-center shrink-0 ml-3 self-center transition-colors
        ...
        <div className="flex gap-2 mb-8">
          {CONTACT_CHANNELS.map(ch => (
            <button
              ...
              className={`h-9 px-4 rounded-full text-[14px] font-medium transition-all active:opacity-70
                ${channel === ch ? 'bg-[#6366F1] text-white' : 'border border-stone-200 text-stone-600 bg-white'}`}
            >
              {ch}
            </button>
          ))}
