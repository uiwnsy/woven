...
export default function InfluencerPerformancePage() {
  ...
  const [view, setView] = useState<'campaign' | 'alltime'>('campaign');
  ...
  const campaignStats = CAMPAIGN_DATA[selectedCampaignId] ?? [];
  ...
  return (
    <div className="flex flex-col h-screen bg-white max-w-[430px] mx-auto relative overflow-hidden">
      ...
      <div className="flex-1 bg-[#FAFBFE] overflow-y-auto pb-10">
        ...
        <div className="bg-white px-5 pt-5 pb-5 flex flex-col gap-4">
          <div className="flex items-center bg-[#EAEDF5] rounded-full p-[3px]">
            {([{ id: 'campaign', label: '캠페인별' }, { id: 'alltime', label: '전체 캠페인' }] as const).map(tab => (
              <button
                key={tab.id}
          ...
          <div className="bg-white px-5 py-6 flex flex-col gap-4">
            ...
            {ALL_TIME.map(inf => (
              <div key={inf.handle} className="bg-[#F8FAFF] border border-[#F5F5F3] rounded-[14px] p-[22px] flex flex-col gap-[10px]">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[13px] font-semibold rounded-full px-[10px] py-[5px]" style={{ backgroundColor: '#EEEEFF', color: '#6366F1' }}>
                    {inf.campaigns}개 캠페인 참여
                  </span>
                  <span className="text-[13px] font-medium truncate" style={{ color: '#B0ADA7' }}>최근: {inf.lastCampaign}</span>
                </div>
                <div className="flex gap-1">
      ...
      {showCampaignSheet && (
        <>
          <div className="absolute inset-0 z-40 bg-black/30" onClick={() => setShowCampaignSheet(false)} />
          <div className="absolute bottom-0 left-0 right-0 z-50 bg-white rounded-t-[20px] pb-8" style={{ paddingTop: 20 }}>
            ...
            <div className="flex flex-col px-5 gap-2">
              {CAMPAIGNS.map(c => {
                ...
                return (
                  <button
                    key={c.id}
                    onClick={() => { setSelectedCampaignId(c.id); setShowCampaignSheet(false); }}
